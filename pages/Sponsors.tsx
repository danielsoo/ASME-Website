...
import { getSponsorContactEmail, getSponsors } from '../src/firebase/services';
...
const Sponsors: React.FC = () => {
  ...
  const getSponsorUrl = (link: string | undefined): string | null => {
          ...
          <div className="container mx-auto max-w-6xl">
              ...
              <div className="bg-[#3b4c6b] p-8 md:p-12 rounded-lg">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                      {sponsorsLoading && (
                        <p className="col-span-2 md:col-span-4 text-center text-white/80 py-8">Loading sponsors...</p>
                      )}
                      {!sponsorsLoading && sponsors.length === 0 && (
                        <p className="col-span-2 md:col-span-4 text-center text-white/80 py-8">No sponsors yet.</p>
                      )}
                      {sponsors.map(sponsor => {
                        const url = getSponsorUrl(sponsor.link);
                        const content = (
                          <>
                            {sponsor.logoUrl ? (
                              <img src={sponsor.logoUrl} alt={sponsor.name} className="w-full h-full object-contain" />
                            ) : (
                              <span className="text-gray-400 font-bold text-sm uppercase tracking-wider text-center px-2">{sponsor.name}</span>
                            )}
                        ...
                        return (
                          <div key={sponsor.id} className="aspect-square bg-white rounded shadow-lg flex items-center justify-center p-4 hover:scale-105 transition-transform duration-300">
                            <div className="relative w-full h-full bg-gray-100 flex items-center justify-center overflow-hidden rounded">
                              {url ? (
                                <a href={url} target="_blank" rel="noopener noreferrer" className="relative block w-full h-full">
                                  {content}
                                </a>
                              ) : (
                                <div className="relative block w-full h-full cursor-default">
                                  {content}
                                </div>
                              )}
                            </div>
